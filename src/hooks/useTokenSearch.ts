  ...
  useEffect(() => {
    const searchTokens = async () => {
      ...
      try {
        ...
        console.log(`[TOKEN-SEARCH] Processing ${topTokens.length} tokens from Moralis in parallel`);

        // Phase 2: Process all tokens in parallel for faster results
        const enhancedResults = await Promise.all(
          topTokens.map(async (moralisToken) => {
            try {
              ...
              // Return token with fallback data even if processing failed
