-- COMPREHENSIVE SECURITY FIX: Address critical data exposure issues
-- Fix multiple security vulnerabilities identified in the security scan

-- 1. Secure token_scans table - users should only see their own scans
...
DROP POLICY IF EXISTS "Service role full access to token_scans" ON token_scans;
DROP POLICY IF EXISTS "Service role full access" ON token_scans;
...
CREATE POLICY "Service role has full access to token_scans"
ON token_scans
FOR ALL
TO service_role
USING (true)
...
-- 2. Secure subscribers table - users should only see their own data
...
-- 3. Secure anonymous_scan_attempts - limit public access
...
-- 4. Update function search paths to fix security warnings
