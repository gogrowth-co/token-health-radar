...
-- First, get the user ID for manga82+3@gmail.com
DO $$
DECLARE
    target_user_id UUID;
...
-- 3. Update RLS policy to allow any admin to call the function
-- The existing policy should work, but let's make sure it's properly set
DROP POLICY IF EXISTS "Authenticated users can call admin functions" ON public.user_roles;

CREATE POLICY "Admins can access user data"
