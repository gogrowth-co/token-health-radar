...
CREATE OR REPLACE FUNCTION public.get_user_hubspot_data(target_user_id uuid)
RETURNS TABLE(
    user_id uuid,
    email text,
    name text,
...
CREATE OR REPLACE FUNCTION public.get_admin_hubspot_data()
RETURNS TABLE(
    user_id uuid,
    email text,
    name text,
...
GRANT EXECUTE ON FUNCTION public.get_user_hubspot_data(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_admin_hubspot_data() TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_user_hubspot_data(uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.get_admin_hubspot_data() TO service_role;