...
async function regenToken(symbol: string): Promise<string> {
  ...
  // populated, so every regenerated snapshot served "—/100" to bots
  // whenever this function fired (e.g. on every run-token-scan
  // completion). Same fix as seo-snapshot/index.ts, 2026-08-05.
  const meta: any = rc?.metadata || {};
  const scores = rc?.scores || meta?.scores || {};
  const input: TokenSnapshotInput = {
    symbol: sym,
    name: report.token_name || cache?.name || meta?.tokenName || sym,
    ...
    what_is: rc?.whatIsToken || null,
    risk_overview: rc?.riskOverview || null,
    security_analysis: rc?.securityAnalysis || null,
    liquidity_analysis: rc?.liquidityAnalysis || null,
    tokenomics_analysis: rc?.tokenomicsAnalysis || null,
    community_analysis: rc?.communityAnalysis || null,
    development_analysis: rc?.developmentAnalysis || null,
    how_to_buy: rc?.howToBuy || null,
    faq: rc?.faq || null,
  };
