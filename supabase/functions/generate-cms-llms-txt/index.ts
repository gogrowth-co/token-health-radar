...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Write to seo-snapshots — the bucket the Cloudflare Worker actually
    // proxies for /llms.txt and /llms-full.txt (bucket mismatch fixed
    // 2026-08-05: writes went to blog-images while the Worker read
    // seo-snapshots, so the served llms.txt was permanently stale).
    // blog-images copies kept for backward compatibility.
    for (const bucket of ["seo-snapshots", "blog-images"]) {
      await supabase.storage.from(bucket).upload("llms.txt", new Blob([llms], { type: "text/plain" }), { contentType: "text/plain; charset=utf-8", upsert: true });
      await supabase.storage.from(bucket).upload("llms-full.txt", new Blob([full], { type: "text/plain" }), { contentType: "text/plain; charset=utf-8", upsert: true });
    }
